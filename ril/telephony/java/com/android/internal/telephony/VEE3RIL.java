...
public class VEE3RIL extends RIL implements CommandsInterface {
    private boolean sentHwBootstrap = false;

    public VEE3RIL(Context context, int networkMode, int cdmaSubscription) {
        super(context, networkMode, cdmaSubscription);
    }

    private static void vee3Sleep(int value) {
    try {
            Thread.sleep(value);
    ...
    lgeSendCommand(int command) {
        ...
        send(rrLSC);
        riljLog("VEE3RIL: LGE COMMAND " + (command) + " sendt");
    }
    ...
    private void restartRild() {
        if (RILJ_LOGD) {
            riljLog("VEE3RIL: restartRild started");
        }
